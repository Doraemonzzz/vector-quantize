...
def generate_llamagen(
    ...
    **sampling_kwargs
    ...
    model_type = (
        model.module.model_type if hasattr(model, "module") else model.model_type
    )
    if model_type == "c2i":
        if cfg_scale > 1.0:
    ...
    with torch.device(device):
        ...
        try:
            ...
            model.module.setup_caches(
                max_batch_size=max_batch_size_cfg,
                max_seq_length=max_seq_length,
                dtype=model.module.tok_embeddings.weight.dtype,
            )
