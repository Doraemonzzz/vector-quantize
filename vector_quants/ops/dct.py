# credit to: https://github.com//zh217/torch-dct
import numpy as np
import torch
...
def dct1_rfft_impl(x):
    dtype = x.dtype
    return torch.view_as_real(torch.fft.rfft(x.float(), dim=1)).to(dtype)


def dct_fft_impl(v):
    dtype = v.dtype
    return torch.view_as_real(torch.fft.fft(v.float(), dim=1)).to(dtype)
