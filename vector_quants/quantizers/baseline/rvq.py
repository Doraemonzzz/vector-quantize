# credit to https://github.com/kakaobrain/rq-vae-transformer
...
class VQEmbedding(nn.Embedding):
    ...
    def _update_embedding(self):
        ...
        num_embed = self.weight.shape[0] - 1
        n = self.cluster_size_ema.sum()
        normalized_cluster_size = (
            n * (self.cluster_size_ema + self.eps) / (n + num_embed * self.eps)
        )
...
class RQBottleneck(nn.Module):
    ...
    Arguments:
        num_embed (int, List, or Tuple): the number of embeddings (i.e., the size of codebook)
            If isinstance(num_embed, int), the sizes of all codebooks are same.
        embed_dim (int): dimension of embed
