...
class FrechetInceptionDistanceDDP(Metric):
    ...
    def compute(self) -> Tensor:
        """DDP version of compute fid"""
        real_features_cov_sum = self.reduce(self.real_features_cov_sum)
        real_features_num_samples = self.reduce(self.real_features_num_samples)
        fake_features_cov_sum = self.reduce(self.fake_features_cov_sum)
        fake_features_num_samples = self.reduce(self.fake_features_num_samples)
        mean_real = (
            self.reduce(self.real_features_sum) / real_features_num_samples
        ).unsqueeze(0)
        mean_fake = (
            self.reduce(self.fake_features_sum) / fake_features_num_samples
        ).unsqueeze(0)
        cov_real_num = (
            real_features_cov_sum
