...
class SGTransformerModel(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        # get params start
        ...
        for i in range(num_layers):
            if i % 2 == 0:
                ...
                cfg.causal = False
            layers.append(TransformerLayer(cfg))
    ...
    def forward_lmhead(self, x):
        if self.tie_word_embeddings:
            output_list = []
            for i, embed in enumerate(self.token_embed):
                output_list.append(F.linear(x[i], embed.weight))
    ...
    def forward(
        ...
        if idx is not None and cond_idx is not None:  # training
            cond_idx = repeat(cond_idx, "b ... -> (g b) ...", g=self.num_group)
        ...
        # (b, *)
        b = hidden_state.shape[0] // self.num_group
        ...
        hidden_state = rearrange(
            hidden_state, "(g b) n ... -> g b n ...", g=self.num_group
        )
        ...
        if LigerFusedLinearCrossEntropyLoss is not None and self.training:
            ...
            for i, lm_head_weight in enumerate(lm_head_weights):
                hidden_state_ = hidden_state[i]
                loss_list.append(
                    loss_fn(
                        lm_head_weight.contiguous().to(dtype),
                        hidden_state_.contiguous()
